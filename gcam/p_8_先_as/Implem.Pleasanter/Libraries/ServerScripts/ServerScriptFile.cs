...
namespace Implem.Pleasanter.Libraries.ServerScripts
{
    public class ServerScriptFile
    {
        ...
        {
            Context = context;
            _fileStorageService = FileStorageServiceFactory.GetService();
        }
            ...
            CallbackHandler(
                ...
                code: () =>
                {
                    var fullPath = NormalizePath(
                        section: section,
                        path: path);
                    if (!_fileStorageService.Exists(fullPath)) return;
                    if (Parameters.Script.ServerScriptFileSizeMax >= 0)
                    {
                        var fileSize = _fileStorageService.GetFileSize(fullPath);
                        if (fileSize >= Parameters.Script.ServerScriptFileSizeMax * 1024 * 1024)
                        {
                            throw new Exception(
                    ...
                    ret = _fileStorageService.ReadAllText(fullPath, GetEncoding(encode: encode));
                });
            return ret;
        }
            ...
            string encode)
        {
            ...
            CallbackHandler(
                ...
                code: () =>
                {
                    ...
                    if (!_fileStorageService.DirectoryExists(dir)) 
                        _fileStorageService.CreateDirectory(dir);
                    _fileStorageService.WriteAllText(fullPath, data, GetEncoding(encode: encode));
                    ret = true;
                });
            return ret;
        }
            ...
            CallbackHandler(
                ...
                code: () =>
                {
                    var fullPath = NormalizePath(
                        section: section,
                        path: path);
                    if (!_fileStorageService.DirectoryExists(fullPath)) return;
                    ret = _fileStorageService.GetFiles(fullPath).Select(x => Path.GetFileName(x)).ToArray();
                });
            ...
            CallbackHandler(
                ...
                code: () =>
                {
                    var fullPath = NormalizePath(
                        ...
                        path: path);
                    if (!_fileStorageService.DirectoryExists(fullPath)) return;
                    ret = _fileStorageService.GetDirectories(fullPath).Select(x => Path.GetFileName(x)).ToArray();
                });
            return V8ScriptEngine.Current.Script.Array.from(ret);
        }
            ...
            CallbackHandler(
                ...
                code: () =>
                {
                    var fullPath = NormalizePath(
                        ...
                        path: path);
                    if (!_fileStorageService.Exists(fullPath)) return;
                    _fileStorageService.Delete(fullPath);
                    ret = true;
                });
            return ret;
        }
            ...
            CallbackHandler(
                ...
                code: () =>
                {
                    ...
                    var newName = NormalizePath(
                        section: section,
                        path: newPath);
                    if (!_fileStorageService.Exists(oldName) || _fileStorageService.Exists(newName)) return;
                    var basePath = Path.GetDirectoryName(newName);
                    if (!_fileStorageService.DirectoryExists(basePath)) 
                        _fileStorageService.CreateDirectory(basePath);
                    _fileStorageService.Move(oldName, newName);
                    ret = true;
                });
            return ret;
        }
            ...
            CallbackHandler(
                ...
                code: () =>
                {
                    var fullPath = NormalizePath(
                        section: section,
                        path: path);
                    if (!_fileStorageService.DirectoryExists(fullPath)) 
                        _fileStorageService.CreateDirectory(fullPath);
                    ret = true;
                });
            return ret;
        }
            ...
            CallbackHandler(
                ...
                code: () =>
                {
                    var fullPath = NormalizePath(
                        ...
                        path: path);
                    if (_fileStorageService.DirectoryExists(fullPath)) 
                        _fileStorageService.DeleteDirectory(fullPath, recursive: true);
                    ret = true;
                });
            return ret;
        }
            ...
            CallbackHandler(
                ...
                code: () =>
                {
                    ...
                    var newName = NormalizePath(
                        section: section,
                        path: newPath);
                    if (!_fileStorageService.DirectoryExists(oldName) || _fileStorageService.DirectoryExists(newName)) return;
                    var basePath = Path.GetDirectoryName(newName);
                    if (!_fileStorageService.DirectoryExists(basePath)) 
                        _fileStorageService.CreateDirectory(basePath);
                    _fileStorageService.MoveDirectory(oldName, newName);
                    ret = true;
                });
            return ret;
        }
