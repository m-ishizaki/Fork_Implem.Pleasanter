...
namespace Implem.Pleasanter.Models
{
    public static class BinaryUtilities
    {
            ...
            long id)
        {
            ...
            var bin = file.Byte();
            if (Parameters.BinaryStorage.IsLocal() || Parameters.BinaryStorage.IsAzureFiles())
            {
                var storageService = FileStorageServiceFactory.GetService();
                var filePath = System.IO.Path.Combine(
                    Directories.BinaryStorage(),
                    "Images",
                    file.Guid);
                storageService.WriteBytes(bin, filePath);
            }
            Repository.ExecuteNonQuery(
                ...
                statements: Rds.InsertBinaries(
                    param: Rds.BinariesParam()
                        ...
                        .BinaryType("Images")
                        .Title(file.FileName)
                        .Bin(bin, _using: !Parameters.BinaryStorage.IsLocal() && !Parameters.BinaryStorage.IsAzureFiles())
